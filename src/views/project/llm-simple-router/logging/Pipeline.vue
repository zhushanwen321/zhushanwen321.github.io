...
<template>
  <div class="prose prose-invert max-w-none">
    <template v-if="isZh">
      <h1>四阶段链路日志</h1>
      <p>Router 为每个请求记录完整的四阶段链路日志，覆盖请求的完整生命周期。</p>
    </template>
    <template v-else>
      <h1>Four-Stage Pipeline Logging</h1>
      <p>Router records complete four-stage pipeline logs for each request, covering the full request lifecycle.</p>
    </template>
    ...
    <p>{{ isZh ? '管理后台 > 请求日志页面。支持按时间范围、状态码、模型名等条件过滤。' : 'Admin Panel > Request Logs page. Filter by time range, status code, model name.' }}</p>
